...
class ManualLEDData:
    ...
    slider_led_tuple: tuple[int, int] = []
...
class SystemLEDData:
    ...
    def update_led_data_for_sending(self, auto_status: bool, manual_status: bool):
        if auto_status and manual_status:
            full_manual_list = self.manual_led_data.generate_full_manual_led_list()
            ...
            missing_leds = find_missing_numbers_as_ranges_tuples(full_led_list)
            print(missing_leds)
            self.turn_off_leds.manual_led_tuple_list = missing_leds
            self.auto_led_data_list = remove_overlapping_ranges_between_auto_led_and_manual_leds(full_manual_list, self.auto_led_data_list)
        elif manual_status:
            full_manual_list = self.manual_led_data.generate_full_manual_led_list()
            missing_leds = find_missing_numbers_as_ranges_tuples(full_manual_list)
            self.turn_off_leds.manual_led_tuple_list = missing_leds
        else:
    ...
    for auto_led in auto_led_data_list:
        for led_range in manual_led_tuple_list:
            if is_overlap(auto_led.led_range, led_range):
                print(led_range, auto_led.led_range)
                auto_leds_to_remove.append(auto_led)
                if auto_led.led_range[0] < led_range[1]:
                    auto_leds_to_return.append(AutoLEDData((led_range[1])))
...
if __name__ == '__main__':
    ...
    print(f"Updated LED Data is: {[print(auto_led.led_range, auto_led.brightness) for auto_led in system_led_data.auto_led_data_list if isinstance(auto_led, AutoLEDData)]}")
