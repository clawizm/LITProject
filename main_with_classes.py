...
def set_command_line_arguments():
    ...
    parser.add_argument("--run_forever", help="Restart Server On Crash", action="store_true")
...
if __name__ == '__main__':
    run_forever_state = set_command_line_arguments()
    subsystem_panels_one = LEDPanels(board.D18)
    subsystem_panels_two = LEDPanels(board.D21)
    subsystem_panel_one_server = LITSubsystemServer(subsystem_panels_one, 5000, run_forever=run_forever_state)
    subsystem_panel_two_server = LITSubsystemServer(subsystem_panels_two, 5001, run_forever=run_forever_state)
